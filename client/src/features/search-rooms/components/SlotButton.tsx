...
export type SlotStatus = "available" | "reserved" | "past" | "selected";
...
export default function SlotButton({ slot, onClick }: SlotButtonProps) {
  ...
  return (
    ...
    >
      ...
      <span className={cn("text-sm font-semibold", isSelected ? "text-primary" : "text-content")}>{slot.time}</span>
      <span
        className={cn(
      ...
      >
        {isPast ? "Past" : isReserved ? "Reserved" : "Available"}
      </span>
    </button>
  );
}
